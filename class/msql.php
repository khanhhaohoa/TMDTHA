...
******************/
    class Mysql{
        ...
        function __construct($host, $user, $pass, $db){
            self::$connection_info = array('host' => 'localhost', 'user' => 'tmdt', 'pass' => 'khanh123', 'db' => 'tmdt');
        }
        ...
        public function search($id, $key, $select = '*'){
            $link =& self::connection();
            if(is_array($select)){
                $cols = '';
                foreach($select as $col){
            ...
            $sql = "SELECT * FROM `donhang` WHERE `name` LIKE '%{$key}%' OR `addrress` LIKE '%{$key}%' OR `phone` LIKE '%{$key}%' OR `link_fb` LIKE '%{$key}%' OR `kieu_dang` LIKE '%{$key}%' OR `ghi_chu_tt` LIKE '%{$key}%' OR `tenmau` LIKE '%{$key}%' OR `id` LIKE '%{$key}%' OR `cod` LIKE '%{$key}%'";
            self::set('last_query', $sql);
        ...
        public function not_like($key, $select = '*'){
            $link =& self::connection();
            if(is_array($select)){
                $cols = '';
                foreach($select as $col){
            ...
            $sql = "SELECT * FROM `product` WHERE `status` = 0 AND `seller_id` NOT LIKE '%{$key}%'";
            self::set('last_query', $sql);
        ...
        public function update_not_like($key, $select = '*'){
            $link =& self::connection();
            if(is_array($select)){
                $cols = '';
                foreach($select as $col){
            ...
            $sql = "SELECT * FROM `product` WHERE `error_update` NOT LIKE '%{$key}%' AND `update_sellerid` NOT LIKE '%{$key}%'";
            self::set('last_query', $sql);
        ...
        public function search_mau($id, $key, $select = '*'){
            $link =& self::connection();
            if(is_array($select)){
                $cols = '';
                foreach($select as $col){
            ...
            $sql = "SELECT * FROM `mockup_count` WHERE `mockup` LIKE '%{$key}%'";
            self::set('last_query', $sql);
        ...
        public function sum($table, $select){
            ...
            $sql = sprintf("SELECT SUM(%s) AS tong FROM %s%s", $select, $table, self::extra());
            self::set('last_query', $sql);
            if(!($result = mysql_query($sql))){
                throw new Exception('Error executing MySQL query: '.$sql.'. MySQL error '.mysql_errno().': '.mysql_error());
                $data = false;
